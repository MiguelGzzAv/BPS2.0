...
async function seed() {
    try {
        ...
        for (const role of mockData.roles) {
            await db.query(
                'INSERT INTO roles (id, company_id, name, is_system_role) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING',
                [role.id, role.company_id, role.name, role.is_system_role]
            );
        }
        console.log('Roles seeded.');

        // Seed Users with new structure
        for (const user of mockData.users) {
            await db.query(
                ...
                [user.id, user.name, user.username, user.password, user.is_superadmin, user.role_id, user.companyId, user.groupIds]
            );
        }
        ...
        // Seed Role Permissions with new structure

            ...
            <div className="mt-3">
                ...
                <div className="row mt-4">
                    <div className="col-md-6">
                        ...
                        <form>
                            <div className="mb-3"><label htmlFor="db-host" className="form-label">Host</label><input type="text" id="db-host" className="form-control" value={config?.host || 'N/A'} disabled /></div>
                            <div className="mb-3"><label htmlFor="db-port" className="form-label">Port</label><input type="text" id="db-port" className="form-control" value={config?.port || 'N/A'} disabled /></div>
                            <div className="mb-3"><label htmlFor="db-user" className="form-label">Username</label><input type="text" id="db-user" className="form-control" value={config?.user || 'N/A'} disabled /></div>
                            <div className="mb-3"><label htmlFor="db-name" className="form-label">Database Name</label><input type="text" id="db-name" className="form-control" value={config?.database || 'N/A'} disabled /></div>
                        </form>
                    </div>
                </div>
            </div>
...
function EnvironmentGuide() {
    ...
    useEffect(() => {
        const fetchGuide = async () => {
            try {
                ...
                if (!response.ok) throw new Error('Failed to fetch the environment guide.');
                const data = await response.json();
    ...
    return (
        <div>
            ...
            <p>This guide shows the content of the <code>.env.example</code> file. Use it as a reference to create or modify your own <code>.env</code> file in the root of the project.</p>
            ...
            <pre className="bg-light p-3 rounded"><code>{guideContent}</code></pre>
        </div>
    );
}
...
function MaintenanceModeSettings() {
    ...
    const handleToggle = (page) => {
        setStatus(prevStatus => ({ ...prevStatus, [page]: !prevStatus[page] }));
    };

    const handleSaveChanges = async () => {
        try {
            setIsSaving(true);
            const response = await fetchWithAuth('/api/maintenance', { method: 'POST', body: status });
                ...
                {Object.keys(status).map(page => (
                    <div key={page} className="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <div className="form-check form-switch">
                            <input className="form-check-input" type="checkbox" role="switch" id={`switch-${page}`} checked={status[page]} onChange={() => handleToggle(page)} disabled={isSaving} />
                            <label className="form-check-label" htmlFor={`switch-${page}`}>{status[page] ? 'Active' : 'Inactive'}</label>
                        </div>
                    </div>
                ))}
            </div>
